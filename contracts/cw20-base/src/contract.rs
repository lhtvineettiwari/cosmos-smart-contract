...
use crate::state::{
    MinterData, TokenInfo, BALANCES, IS_ALLOWED_ADDRESS, IS_PAUSED, LOGO, MARKETING_INFO, OWNER,
    TOKEN_INFO,
};
...
pub fn instantiate(
    ...
    IS_ALLOWED_ADDRESS.save(deps.storage, &_info.sender.clone(), &true)?;
    ...
    //Marking minter as IS_ALLOWED address, if singer, minter addr are diff
    if config.mint.as_ref().unwrap().minter != _info.sender {
        IS_ALLOWED_ADDRESS.save(
            deps.storage,
            &config.mint.as_ref().unwrap().minter.clone(),
            &true,
        )?;
    }
...
pub fn add_to_deny_list(
    ...
    if OWNER.load(deps.storage)? != _info.sender {
        return Err(ContractError::Unauthorized {});
    }

    if IS_PAUSED.may_load(deps.storage)?.unwrap() {
        return Err(ContractError::Unauthorized {});
    }

    let rcpt_addr = deps.api.addr_validate(&recipient)?;

    IS_ALLOWED_ADDRESS.save(deps.storage, &rcpt_addr, &false)?;

    let res = Response::new()
        .add_attribute("action", "added_to_denied_list")
        .add_attribute("admin", _info.sender.to_string())
        .add_attribute("denied_addr", recipient);
...
pub fn add_to_allow_list(
    ...
    IS_ALLOWED_ADDRESS.save(deps.storage, &rcpt_addr, &true)?;

    let res = Response::new()
        .add_attribute("action", "added_to_allowed_list")
        .add_attribute("admin", _info.sender.to_string())
        .add_attribute("allowed_addr", recipient);

    Ok(res)
}

pub fn pause(deps: DepsMut, _info: MessageInfo) -> Result<Response, ContractError> {
    if OWNER.load(deps.storage)? != _info.sender {
        return Err(ContractError::Unauthorized {});
    }
    ...
    let res = Response::new()
        ...
        .add_attribute("from", _info.sender.to_string())
        .add_attribute("is_paused", "true");
    Ok(res)
}

pub fn unpause(deps: DepsMut, _info: MessageInfo) -> Result<Response, ContractError> {
    if OWNER.load(deps.storage)? != _info.sender {
        return Err(ContractError::Unauthorized {});
    }
...
pub fn execute(
    ...
    match msg {
        ...
        ExecuteMsg::AddToAllowList { recipient } => add_to_allow_list(deps, info, recipient),
        ExecuteMsg::AddToDenyList { recipient } => add_to_deny_list(deps, info, recipient),
...
pub fn execute_update_minter(
    ...
    config.mint = Some(data);

    TOKEN_INFO.save(deps.storage, &config)?;

    let res = Response::new()
        .add_attribute("action", "update_minter")
        .add_attribute("new_minter_addr", new_minter);
    Ok(res)
}
...
pub fn execute_mint(
    ...
    IS_ALLOWED_ADDRESS.save(deps.storage, &rcpt_addr.clone(), &true)?;
...
pub fn execute_send(
    ...
    if IS_ALLOWED_ADDRESS
        ...
        == false
        || IS_ALLOWED_ADDRESS
            .may_load(deps.storage, &rcpt_addr.clone())?
            .unwrap()
            == false
    {
        return Err(ContractError::Unauthorized {});
